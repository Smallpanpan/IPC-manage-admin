<template>
  <div class="croppa-div">
    ...
    <img v-if="isUpload" :src="accept" class="upload-icon" @click="upload">
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onFileChoose(file) {
      this.hasChange = true
      if (this.isShowUpload) { this.isUpload = true }
    ...
    async upload() {
      ...
      if (this.required) {
        const blob = await this.myCroppa.promisedBlob('image/jpeg', 1)
        if (!blob) {
          this.$message({
            message: '请选择1张图片',
            type: 'error'
        ...
        const data = await upload(this.myCroppa.chosenFile.name, blob)
        this.isUpload = false
