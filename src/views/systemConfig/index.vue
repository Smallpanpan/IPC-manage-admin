        ...
        <keep-alive>
          <el-form ref="baseConfig" :model="baseConfig" label-width="160px" style="width: 600px;">
            <el-form-item label="关闭网站">
              <el-switch v-model="baseConfig.isClose"/>
            </el-form-item>
            <el-form-item label="关闭原因">
              <el-input v-model="baseConfig.closeReason" type="textarea"/>
            </el-form-item>
            <el-form-item label="关闭注册">
              <el-switch v-model="baseConfig.isRegister"/>
            </el-form-item>
            <el-form-item label="ICP备案证书号">
              <el-input v-model="baseConfig.ICPNO"/>
            </el-form-item>
            <el-form-item label="市场价格比例">
              <el-input v-model="baseConfig.marketPriceRatio"/>
            </el-form-item>
            <el-form-item label="消费积分名称">
              <el-input v-model="baseConfig.consumptionPointsName"/>
            </el-form-item>
            <el-form-item label="积分换算比例">
              <el-input v-model="baseConfig.integralConversionRatio"/>
            </el-form-item>
            <el-form-item label="积分支付比例">
              <el-input v-model="baseConfig.integralPaymentRatio"/>
            </el-form-item>
            <el-form-item label="商品货号前缀">
              <el-input v-model="baseConfig.shopNumberPrefix"/>
            </el-form-item>
            <el-form-item label="用户评论是否需要审核">
              <el-switch v-model="baseConfig.isCommentReviewed"/>
            </el-form-item>
            <el-form-item label="会员注册赠送积分">
              <el-input v-model="baseConfig.registerPresentExp"/>
            </el-form-item>
            <el-form-item>
        ...
        <keep-alive>
          <el-form ref="shop" :model="shop" label-width="160px" style="width: 600px;">
            <el-form-item label="寄件人姓名">
              <el-input v-model="shop.sender"/>
            </el-form-item>
            <el-form-item label="商店名称">
              <el-input v-model="shop.shopName"/>
            </el-form-item>
            <el-form-item label="商店标题">
              <el-input v-model="shop.shopTitle" placeholder="商店的标题将显示在浏览器的标题栏"/>
            </el-form-item>
            <el-form-item label="商店描述">
              <el-input v-model="shop.shopDescribe"/>
            </el-form-item>
            <el-form-item label="商店关键字">
              <el-input v-model="shop.shopKeyWord"/>
            </el-form-item>
            ...
            <el-form-item label="详细地址">
              <el-input v-model="shop.completeAddress"/>
            </el-form-item>
            <el-form-item label="客服QQ">
              <el-input v-model="shop.serviceQQ"/>
            </el-form-item>
            <el-form-item label="邮件地址">
              <el-input v-model="shop.serviceEmail"/>
            </el-form-item>
            <el-form-item label="客服电话">
              <el-input v-model="shop.serviceCall"/>
            </el-form-item>

            <el-form-item label="商店 Logo(推荐分辨率180x90)">
              <Croppa ref="shopLogo" v-model="shop.shopLogo" :width="180" :height="90"/>
            </el-form-item>
...
export default {
  ...
  methods: {
    handleItemChange(vals) {
      const val_obj = vals[vals.length - 1]
      const val = val_obj.region_id
      if (!citis_arr[val]) {
        const obj = this.getDataIndex(Number(val), this.region)
        if (obj) {
          citis_arr[val] = obj
          if (obj.cities && obj.cities.length > 0) { return }
    ...
    getDataIndex(val, data) {
      for (const v of data) {
        if (v.region_id == val) { return v }
        if (v.cities && v.cities.length > 0) {
          const res = this.getDataIndex(val, v.cities)
          if (res) { return res }
        }
      }
    },
    ...
    async onSubmit() {
      const res = await this.$refs['shopLogo'].upload()
      if (res) {
